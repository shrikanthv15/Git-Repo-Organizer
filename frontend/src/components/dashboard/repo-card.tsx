...
export function RepoCard({ repo, onClick }: RepoCardProps) {
    ...
    // Map backend Repo to display values
    const healthScore = repo.health?.health_score ?? 0;
    const hasHealth = repo.health != null;
    const hasPendingPR = !!repo.health?.pending_fix_url;

    // Use last_commit_date from health if available
    const lastUpdated = repo.health?.last_commit_date
        ? new Date(repo.health.last_commit_date).toLocaleDateString()
        : "Not analyzed";
            ...
            <div className={cn(
                ...
                <div className="flex items-start justify-between">
                    <div className="flex-1">
                        ...
                        <p className="mt-1 text-sm text-muted-foreground line-clamp-1">{repo.description || repo.full_name}</p>
                    </div>
                    {hasHealth ? (
